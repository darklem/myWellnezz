...
class Event:
    def __init__(self, **kwargs):
        try:
            ...
            self.booking_opens_on: datetime = datetime.strptime(str(self.partition_date),"%Y%m%d")-timedelta(days=14) if kwargs.get('bookingOpensOn') is None else parser.parse(kwargs.get('bookingOpensOn')).replace(tzinfo=None)
